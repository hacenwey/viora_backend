...
@section('title',settings()->get('app_name').' | '. trans('global.edit').' '. trans('cruds.collection.title_singular'))
...
@section('main-content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="post" class="row" action="{{route('backend.collections.update', ['collection' => $collection->id])}}">
                {{ csrf_field() }}
                @method('PUT')

                <div class="form-group col-md-8">
                    <label for="inputTitle" class="col-form-label">@lang('cruds.collection.fields.title') <span class="text-danger">*</span></label>
                    <input id="inputTitle" type="text" name="title" placeholder="@lang('global.enter') @lang('cruds.collection.fields.title')"  value="{{ old('title', $collection->title) }}" class="form-control">
                    @error('title')
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="status" class="form-control">
                        <option value="active" {{ $collection->status == 'active' ? 'selected' : '' }}>@lang('global.active')</option>
                        <option value="inactive" {{ $collection->status == 'inactive' ? 'selected' : '' }}>@lang('global.inactive')</option>
                    </select>
                    @error('status')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control" id="summary" name="summary">{{old('summary', $collection->summary)}}</textarea>
                    @error('summary')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-12">
                    ...
                    <textarea class="form-control" id="description" name="description">{{old('description', $collection->description)}}</textarea>
                    @error('description')
                    <span class="text-danger">{{$message}}</span>
                    @enderror
                </div>
                ...
                <div class="form-group col-md-12">
                    ...
                    <div class="input-group">
                        ...
                        <input id="thumbnail" class="form-control" type="text" name="photo" value="{{old('photo', $collection->photo)}}">
                    </div>

                    <div id="holder" style="margin-top:15px;max-height:100px;">
                        @if($collection->photo != null)
                            <img src="{{ $collection->photo }}" alt="" width="200">
                        @endif
                    </div>
                    @error('photo')
                    <span class="text-danger">{{$message}}</span>
                    ...
                    <div class="dynamicSelect">
                        <div class="row">
                            ...
                            <div class="col-md-8" id="productType">
                                ...
                                <select name="products[]" id="prodType" class="form-control select2 {{ $collection->type == 'category' ? 'd-none' : '' }}" multiple>
                                    @foreach($products as $key => $product)
                                        <option value="{{ $product->id }}" {{ in_array($product->id, $collection->products->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $product->title }}</option>
                                    @endforeach
                                </select>
                                <select name="categories[]" id="catType" class="form-control select2 {{ $collection->type == 'product' ? 'd-none' : '' }}" multiple>
                                    @foreach($categories as $key => $category)
                                        <option value="{{ $category->id }}" {{ in_array($category->id, $collection->categories->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $category->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
