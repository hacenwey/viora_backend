...
@section('title',settings()->get('app_name').' | ' . trans('cruds.user.title_singular') . trans('global.profile'))
...
<div class="card shadow mb-4">
   ...
   <div class="card-header py-3">
     <h4 class=" font-weight-bold">@lang('global.profile')</h4>
     <ul class="breadcrumbs">
         <li><a href="{{route('backend.admin')}}" style="color:#999">@lang('global.dashboard')</a></li>
         <li><a href="" class="active text-primary">@lang('global.profile')</a></li>
     </ul>
   </div>
   <div class="card-body">
        <div class="row">
            ...
            <div class="col-md-8">
                <form class="border px-4 pt-2 pb-3" method="POST" action="{{route('backend.profile-update',$profile->id)}}">
                    @csrf
                    <div class="form-group">
                        <label for="inputTitle" class="col-form-label">@lang('cruds.user.fields.name')</label>
                      <input id="inputTitle" type="text" name="name" placeholder="@lang('global.enter') @lang('cruds.user.fields.name')"  value="{{$profile->name}}" class="form-control">
                      @error('name')
                      ...
                      <div class="form-group">
                        ...
                        <input id="inputEmail" disabled type="email" name="email" placeholder="@lang('global.enter') @lang('cruds.user.fields.email')"  value="{{$profile->email}}" class="form-control">
                        @error('email')
                        <span class="text-danger">{{$message}}</span>
                        @enderror
                      </div>
                      ...
                      <div class="form-group">
                          ...
                          <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple required>
                              @foreach($roles as $id => $roles)
                                <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $profile->roles->contains($id)) ? 'selected' : '' }}>{{ $roles }}</option>
                              @endforeach
                          </select>
                          @error('role')
                            <span class="text-danger">{{$message}}</span>
