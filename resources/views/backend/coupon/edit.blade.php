...
@section('title',settings()->get('app_name').' | '. trans('global.edit').' '. trans('cruds.coupon.title_singular'))

@section('main-content')

<div class="card">
    <h5 class="card-header">@lang('global.edit') @lang('cruds.coupon.title_singular')</h5>
    <div class="card-body">
        <form method="post" class="row" action="{{route('backend.coupon.update',$coupon->id)}}">
            ...
            <div class="form-group col-md-4">
                ...
                <div class="input-group mb-3">
                    <input id="inputTitle" type="text" name="code"
                        ...
                        aria-describedby="basic-addon2" value="{{ $coupon->code }}" class="form-control code-input">
            ...
            <div class="form-group col-md-4">
                ...
                <select name="type" class="form-control">
                    <option value="fixed" {{(($coupon->type=='fixed') ? 'selected' : '')}}>
                        @lang('cruds.coupon.fields.fixed')</option>
                    <option value="percent" {{(($coupon->type=='percent') ? 'selected' : '')}}>
                        @lang('cruds.coupon.fields.percent')</option>
                </select>
                @error('type')
                <span class="text-danger">{{$message}}</span>
                @enderror
            ...
            <div class="form-group col-md-4">
                <label for="inputTitle" class="col-form-label">@lang('cruds.coupon.fields.value') <span
                        class="text-danger">*</span></label>
                <input id="inputTitle" type="number" name="value" placeholder="@lang('cruds.coupon.fields.value')"
            ...
            <div class="form-group col-md-4">
                ...
                <input id="inputQty" type="number" name="quantity"
                    ...
                    value="{{old('quantity', $coupon->code)}}" class="form-control">
                @error('quantity')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <input id="inputDate" type="text" name="expires_at"
                    ...
                    value="{{old('expires_at', $coupon->expires_at)}}" class="form-control datetime">
                @error('expires_at')
                <span class="text-danger">{{$message}}</span>
                @enderror
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="status" class="form-control">
                    <option value="active" {{(($coupon->status=='active') ? 'selected' : '')}}>@lang('global.active')
                    </option>
                    <option value="inactive" {{(($coupon->status=='inactive') ? 'selected' : '')}}>
                        @lang('global.inactive')</option>
                </select>
                @error('status')
                <span class="text-danger">{{$message}}</span>
                @enderror
