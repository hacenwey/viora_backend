...
@section('title',settings()->get('app_name').' | '. trans('global.edit').' '. trans('cruds.banner.title_singular'))
@section('main-content')

<div class="card">
    <h5 class="card-header">@lang('global.edit') @lang('cruds.banner.title_singular')</h5>
    <div class="card-body">
      <form method="post" class="row" action="{{route('backend.banner.update',$banner->id)}}">
        @csrf
        @method('PATCH')
        <div class="form-group col-md-8">
          <label for="inputTitle" class="col-form-label">@lang('cruds.banner.fields.title') <span class="text-danger">*</span></label>
        <input id="inputTitle" type="text" name="title" placeholder="@lang('global.add') @lang('cruds.banner.fields.title')"  value="{{$banner->title}}" class="form-control">
        @error('title')
        ...
        @enderror
          ...
          <div class="form-group col-md-4">
              ...
              <select name="status" id="status" class="form-control">
                  <option value="active" {{(($banner->status=='active') ? 'selected' : '')}}>@lang('global.active')</option>
                  <option value="inactive" {{(($banner->status=='inactive') ? 'selected' : '')}}>@lang('global.inactive')</option>
              </select>
              @error('status')
              <span class="text-danger">{{$message}}</span>
              @enderror
        ...
        <div id="holder" style="margin-top:15px;max-height:100px;"></div>
          ...
          @if($banner->link != null)
              <div class="col-md-12">
                  <label for="oldLink">@lang('global.old_link')</label>
                  <br>
                  <a href="{{ $banner->link }}" target="_blank" id="oldLink">{{ $banner->link }}</a>
              </div>
          @endif
